...
class TestGoalJudgeTimeout:
    ...
    config value flows through to ``call_llm(timeout=...)`` (the goal judge's
    auxiliary entry point) and that a missing config falls back to the default.
    """

    def test_config_timeout_flows_to_create_call(self, hermes_home):
        from hermes_cli import goals

        with patch("agent.auxiliary_client.call_llm") as call, patch(
